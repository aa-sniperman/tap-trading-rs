...
impl LedgerRepository {
    ...
    async fn append_entry(
        ...
        self.sync_balance_cache(&balance).await;
    ...
    async fn bump_balances_recording_mode(
        ...
        Self::map_account_balance_row(&row)
    }
    ...
    fn map_account_balance_row(row: &sqlx::postgres::PgRow) -> Result<AccountBalance, LedgerError> {
        Ok(AccountBalance {
            user_id: row
                .try_get("user_id")
                .map_err(|error| LedgerError::Repository(error.to_string()))?,
    ...
    async fn sync_balance_cache(&self, balance: &AccountBalance) {
        if let Err(error) = self.balance_cache.set_balance(balance).await {
            warn!(
                user_id = %balance.user_id,
                asset = balance.asset,
                error = %error,
                "failed to refresh balance cache after db commit"
            );
        }
    }
}
...
impl crate::domain::ledger::LedgerRepository for LedgerRepository {
    async fn ensure_account_balance(&self, user_id: UserId, asset: &str) -> Result<(), LedgerError> {
        ...
        let balance = Self::map_account_balance_row(&row)?;
        self.sync_balance_cache(&balance).await;
